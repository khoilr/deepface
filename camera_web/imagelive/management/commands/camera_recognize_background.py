...
def detect_faces(frame):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for face in face_objs:
            ...
            futures.append((new_row, future))

        # Wait for all background tasks to complete and process their results
        for new_row, future in futures:
            face_id = future.result()
            new_row["Face ID"] = face_id
            df_faces.loc[len(df_faces)] = new_row
...
class Command(BaseCommand):
    help = "Starts a background process in a separate thread"

    def handle(self, *args, **kwargs):
        thread = threading.Thread(target=background_process)
        thread.daemon = True
        thread.start()
        self.stdout.write(self.style.SUCCESS("Background process started in a separate thread."))
