from custom_deepface.face_detection.load_weights import load_weights
...
def build_model():
    data = Input(dtype=float32, shape=(None, None, 3), name="data")
    ...
    x1_shape = shape(ssh_c3_up)
    x2_shape = shape(ssh_c2_lateral_relu)
    offsets = [0, (x1_shape[1] - x2_shape[1]) // 2, (x1_shape[2] - x2_shape[2]) // 2, 0]
    size = [-1, x2_shape[1], x2_shape[2], -1]
    crop0 = slice(ssh_c3_up, offsets, size, "crop0")
    ...
    ssh_c2_aggr = Conv2D(
        filters=256, kernel_size=(3, 3), name="ssh_c2_aggr", strides=[1, 1], padding="VALID", use_bias=True
    )(ssh_c2_aggr_pad)
    ...
    x1_shape = shape(ssh_m2_red_up)
    x2_shape = shape(ssh_m1_red_conv_relu)
    offsets = [0, (x1_shape[1] - x2_shape[1]) // 2, (x1_shape[2] - x2_shape[2]) // 2, 0]
    size = [-1, x2_shape[1], x2_shape[2], -1]
    crop1 = slice(ssh_m2_red_up, offsets, size, "crop1")
    ...
    ssh_c1_aggr = Conv2D(
        filters=256, kernel_size=(3, 3), name="ssh_c1_aggr", strides=[1, 1], padding="VALID", use_bias=True
    )(ssh_c1_aggr_pad)
    ...
    final = stack([inter_1, inter_2])
    face_rpn_cls_score_reshape_stride32 = transpose(final, (1, 2, 3, 0), name="face_rpn_cls_score_reshape_stride32")
    ...
    input_shape = [shape(face_rpn_cls_prob_stride32)[k] for k in range(4)]
    sz = dtypes.cast(input_shape[1] / 2, dtype=int32)
    ...
    final = stack([inter_1, inter_3, inter_2, inter_4])
    face_rpn_cls_prob_reshape_stride32 = transpose(final, (1, 2, 3, 0), name="face_rpn_cls_prob_reshape_stride32")
    ...
    final = stack([inter_1, inter_2])
    face_rpn_cls_score_reshape_stride16 = transpose(final, (1, 2, 3, 0), name="face_rpn_cls_score_reshape_stride16")
    ...
    input_shape = [shape(face_rpn_cls_prob_stride16)[k] for k in range(4)]
    sz = dtypes.cast(input_shape[1] / 2, dtype=int32)
    ...
    final = stack([inter_1, inter_3, inter_2, inter_4])
    face_rpn_cls_prob_reshape_stride16 = transpose(final, (1, 2, 3, 0), name="face_rpn_cls_prob_reshape_stride16")
    ...
    final = stack([inter_1, inter_2])
    face_rpn_cls_score_reshape_stride8 = transpose(final, (1, 2, 3, 0), name="face_rpn_cls_score_reshape_stride8")
    ...
    input_shape = [shape(face_rpn_cls_prob_stride8)[k] for k in range(4)]
    sz = dtypes.cast(input_shape[1] / 2, dtype=int32)
    ...
    final = stack([inter_1, inter_3, inter_2, inter_4])
    face_rpn_cls_prob_reshape_stride8 = transpose(final, (1, 2, 3, 0), name="face_rpn_cls_prob_reshape_stride8")
    model = Model(
        inputs=data,
        outputs=[
